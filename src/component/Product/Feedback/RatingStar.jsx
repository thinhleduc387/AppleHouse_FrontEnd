...
const RatingStar = ({ spuId = null, numberOfRating, haveNewRating }) => {
  ...
  return (
    <div className="flex flex-col p-6 bg-white dark:bg-gray-800 rounded-xl w-full mx-auto">
      ...
      <div className="space-y-4">
        {ratingPercentages.map((ratingData, index) => (
          <div className="flex items-center group" key={index}>
            <p className="w-8 text-sm font-bold text-gray-800 dark:text-gray-100">
              {ratingData.score}.0
            </p>
            <svg
              className="w-5 fill-blue-600 dark:fill-blue-400 transform group-hover:scale-110 transition-transform"
              viewBox="0 0 14 13"
            ...
            <p className="w-12 text-sm font-bold text-gray-800 dark:text-gray-100">
              {ratingData.percent}
            </p>
          </div>
        ))}
